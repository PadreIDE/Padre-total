package Padre::Plugin::SpellCheck::FBP::Dialog;
...
sub new {
	...
	Wx::Event::EVT_BUTTON(
		...
		$self->{replaece_all},
		sub {
			shift->_on_butreplace_all_clicked(@_);
		},
	);
	...
	Wx::Event::EVT_BUTTON(
		...
		$self->{ignore_all},
		sub {
			shift->_on_butignore_all_clicked(@_);
		},
	);
	...
	$sbSizer1->Add( $self->{labeltext}, 0, Wx::ALL | Wx::EXPAND, 5 );
	$sbSizer1->Add( $self->{label}, 0, Wx::ALL | Wx::EXPAND, 5 );

	my $bSizer21 = Wx::BoxSizer->new(Wx::VERTICAL);
	$bSizer21->Add( $sbSizer1, 1, Wx::ALL | Wx::EXPAND, 5 );
...
sub _on_butreplace_all_clicked {
	$_[0]->main->error('Handler method _on_butreplace_all_clicked for event replaece_all.OnButtonClick not implemented');
}

sub _on_butignore_clicked {
	$_[0]->main->error('Handler method _on_butignore_clicked for event ignore.OnButtonClick not implemented');
}

sub _on_butignore_all_clicked {
	$_[0]->main->error('Handler method _on_butignore_all_clicked for event ignore_all.OnButtonClick not implemented');
}
