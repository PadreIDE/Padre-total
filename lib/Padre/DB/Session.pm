...
=head2 base
  ...
  # Returns 'Padre::DB'
  my $namespace = Padre::DB::Session->base;
...
=head2 load
  ...
  my $object = Padre::DB::Session->load( $id );
...
It returns a C<Padre::DB::Session> object, or throws an exception if the
...
C<FROM session> section of the query, followed by variables
to be bound to the placeholders in the SQL phrase. Any SQL that is
compatible with SQLite can be used in the parameter.

Returns a list of B<Padre::DB::Session> objects when called in list context, or a
reference to an C<ARRAY> of B<Padre::DB::Session> objects when called in scalar
context.
...
following, except with an O(1) memory cost instead of O(n).
  ...
  foreach ( Padre::DB::Session->select ) {
      print $_->id . "\n";
  }

You can filter the list via SQL in the same way you can with C<select>.

  Padre::DB::Session->iterate(
      'order by ?', 'id',
      sub {
          print $_->id . "\n";
      }
...
references when used in this form.
  ...
  Padre::DB->iterate(
      'select name from session order by id',
      sub {
          print $_->[0] . "\n";
      }
  );
...
Returns a new L<Padre::DB::Session> object.

=head2 create

  my $object = Padre::DB::Session->create(
...
L<Padre::DB::Session> object, inserts the appropriate row into the
L<session> table, and then returns the object.
...
Returns a new L<session> object, or throws an exception on
...
=head2 delete
  ...
  # Delete multiple rows from the session table
  Padre::DB::Session->delete('where id > ?', 1000);

The C<delete> method can be used in a class form and an instance form.

When used on an existing B<Padre::DB::Session> instance, the C<delete> method
removes that specific instance from the C<session>, leaving
...
phrase to be added after the C<DELETE FROM session> section
...
=head2 truncate
  ...
  # Delete all records in the session table
...
The session table was originally created with the
...
Padre::DB::Session is part of the L<Padre::DB> API.
