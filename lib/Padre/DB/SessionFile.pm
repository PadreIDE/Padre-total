...
This class allows storing in L<Padre>'s database the files referenced by a
given session (see C<Padre::DB::Session>).
...
It returns a C<Padre::DB::SessionFile> object, or throws an exception if the
...
C<FROM session_file> section of the query, followed by variables
to be bound to the placeholders in the SQL phrase. Any SQL that is
compatible with SQLite can be used in the parameter.

Returns a list of B<Padre::DB::SessionFile> objects when called in list context, or a
reference to an C<ARRAY> of B<Padre::DB::SessionFile> objects when called in scalar
context.
...
Returns a new L<Padre::DB::SessionFile> object.
...
L<Padre::DB::SessionFile> object, inserts the appropriate row into the
L<session_file> table, and then returns the object.
...
Returns a new L<session_file> object, or throws an exception on
...
When used on an existing B<Padre::DB::SessionFile> instance, the C<delete> method
removes that specific instance from the C<session_file>, leaving
...
phrase to be added after the C<DELETE FROM session_file> section
...
The session_file table was originally created with the
...
Padre::DB::SessionFile is part of the L<Padre::DB> API.
