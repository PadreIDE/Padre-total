...
sub new {
   ...
   my $path = File::Spec->catfile( Padre::Constant::CONFIG_DIR, 'lwp_cookies.dat');
...
sub register {
   ...
   my $server = $self->config->cs_default_server;
...
sub local_to_server {
   ...
   my $conf = Padre->ide->config->human;
   ...
   my $resp = $self->ua->request( POST "$server/user/config", 'Content-Type' => 'application/json', 'Content' => $self->_objToJson( \%h ) );
