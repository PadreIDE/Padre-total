...
sub new {
   ...
   my $config_sync = Padre->ide->config_sync;
	...
	$self->{txt_login} = Wx::TextCtrl->new(
		$self->{Login_Pane},
		-1,
		$config->cs_username ? $config->cs_username : '',
		Wx::wxDefaultPosition,
		Wx::wxDefaultSize,
	);
	...
	$self->{btn_login} = Wx::Button->new(
		$self->{Login_Pane},
		-1,
		Padre->ide->config_sync->{state} eq 'logged_in' ? 'Log out' : 'Log in',
	);
	...
	$self->{txt_remote} = Wx::TextCtrl->new(
		$self->{Settings_Pane},
		-1,
		$config->cs_default_server ? $config->cs_default_server : '',
		Wx::wxDefaultPosition,
		Wx::wxDefaultSize,
	);
...
sub btn_login { 
   ...
   my $cs       = Padre->ide->config_sync;

   # handle login / logout logic toggle
   if ($cs->{state} eq 'logged_in') { 
      if ($cs->logout =~ /success/) { 
         Wx::MessageBox(
      ...
      $self->{lbl_status}->SetLabel( $cs->english_status );
      $self->{lbl_status_info}->SetLabel( $cs->english_status );
      return;
   }
