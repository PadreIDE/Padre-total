...
sub dialog {
    ...
    Wx::Event::EVT_BUTTON( $dialog, $dialog->{_widgets_}->{_tidy_all_},
        \&tidy_all );
    Wx::Event::EVT_BUTTON( $dialog,
        $dialog->{_widgets_}->{_tidy_selection_},
        \&tidy_selection );
    Wx::Event::EVT_BUTTON( $dialog, $dialog->{_widgets_}->{_cancel_},
        \&cancel_clicked );
...
sub tidy_all {
    ...
    if ( open my $fh, '>', $perltidyrc ) {
        ...
        warn
"Could not create temporary rc file for perltidy ($perltidyrc) $!";
    }
}
