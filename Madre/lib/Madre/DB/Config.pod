...
=head2 load
  ...
  my $object = Madre::DB::Config->load( $config_id );
...
It returns a C<Madre::DB::Config> object, or throws an exception if the
...
=head2 select
  ...
  my $array_ref = Madre::DB::Config->select(
      'where config_id > ? order by config_id',
      1000,
  );
...
references when used in this form.
  ...
  Madre::DB->iterate(
      'select name from config order by config_id',
      sub {
          print $_->[0] . "\n";
      }
  );
...
Returns a new L<Madre::DB::Config> object.

=head2 create

  my $object = Madre::DB::Config->create(
      config_id => 'value',
      user_id => 'value',
      data => 'value',
      modified => 'value',
...
L<Madre::DB::Config> object, inserts the appropriate row into the
L<config> table, and then returns the object.

If the primary key column C<config_id> is not provided to the
constructor (or it is false) the object returned will have
C<config_id> set to the new unique identifier.
 
Returns a new L<config> object, or throws an exception on
...
If a the primary key column C<config_id> is not provided to the
constructor (or it is false) the object returned will have
C<config_id> set to the new unique identifier.
...
=head2 delete
  ...
  # Delete multiple rows from the config table
  Madre::DB::Config->delete('where config_id > ?', 1000);

The C<delete> method can be used in a class form and an instance form.

When used on an existing B<Madre::DB::Config> instance, the C<delete> method
removes that specific instance from the C<config>, leaving
...
phrase to be added after the C<DELETE FROM config> section
...
=head2 truncate
  ...
  # Delete all records in the config table
  Madre::DB::Config->truncate;
