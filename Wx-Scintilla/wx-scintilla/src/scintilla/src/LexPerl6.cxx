...
static void ColourisePerl6Doc(unsigned int startPos, int length, int, WordList *keywordLists[], Accessor &styler)
{
	...
	for (int i = startPos; i < lengthDoc; i++) {
		...
		switch(state) {
			case SCE_PERL6_DEFAULT:
				if( ch == '\n' || ch == '\r' || ch == '\t' || ch == ' ') {
					// Whitespace is ignored here
					styler.ColourTo(i,SCE_PERL6_DEFAULT);
					break;
				} else if( ch == '#' ) {
					// The start of a comment
					state = SCE_PERL6_COMMENT;
					styler.ColourTo(i,SCE_PERL6_COMMENT);
				} else if( ch == '"') {
					// The start of a string
					state = SCE_PERL6_STRING;
					styler.ColourTo(i,SCE_PERL6_STRING);
				} else {
					// The default style..
					styler.ColourTo(i,SCE_PERL6_DEFAULT);
				}
