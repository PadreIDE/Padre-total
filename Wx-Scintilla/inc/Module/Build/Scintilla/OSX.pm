...
sub stc_install_scintilla_library {
    ...
    qx($updateidcommand);
    {
        ...
        if(!$success) {
            warn("Could not confirm ID update for $libname after ID update complete. Shared library may fail in distribution");
            chmod($storedmode, $targetfile);
            return;
        }
    }
    ...
    my $libidcommand = 'otool -LX "' . $targetfile . '"| grep -e ' . qq('$fworkprefix') . ' | cut -d " " -f 1';
    {
        ...
        for my $oline (@outputlines) {
            ...
            my $updatedepencycommand = qq(install_name_tool -change \"$originaldependencypath\" \"$newdependencypath\" \"$targetfile\");
